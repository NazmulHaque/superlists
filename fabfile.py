...
REPO_URL = 'https://github.com/NazmulHaque/superlists'
env.user = 'ubuntu'
env.password = 'qweqwe'
...
def _update_virtualenv(source_folder):
    virtualenv_folder = source_folder + '/../virtualenv/venv'
...
def _update_static_files(source_folder):
    run('cd %s && ../virtualenv/venv/bin/python manage.py collectstatic --noinput' % (
        source_folder,
    ))

def _update_database(source_folder):
    run('cd %s && ../virtualenv/venv/bin/python manage.py migrate --noinput' % (
        source_folder,
    ))
